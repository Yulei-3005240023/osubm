...
class SubsidenceModel:
    ...
    7 有一个非常奇葩且重要的假设条件就是单个时间步长内完成固结，所以时间步长既不能太大也不能太小
    """
    ...
    def __init__(self, L = 10.0, N = 10, T = 10, dt = 0.5, delta_pwp = 98100, z0=40, external_load=0):
        ...
        self.press_period = 10 # 初始设置有10个应力期
        self.M_press_period = int(self.M / self.press_period) #单个应力期内的时间步个数
        ...
        self.Delta_pwp_press_period = np.zeros(self.press_period)  # 这个是应力期的外加应力储存
    ...
    def delta_pwp_redefine(self):
        ...
        for m in range(0, self.M):
            ...
            if m < int(self.M / 5):
                self.Delta_pwp[m] = self.delta_pwp0
            elif  int(self.M / 5)<= m < int(self.M / 4):
                self.Delta_pwp[m] = 0
            else:
                self.Delta_pwp[m] = 0#self.delta_pwp0

    def delta_pwp_redefine_stress_period(self):
        # 随便测试一下在T=0.5T时卸载压力会是什么样子
        for m in range(0, self.press_period):
            #self.Delta_pwp[m] = self.delta_pwp0
            if m < int(self.press_period/ 5):
                self.Delta_pwp_press_period[m] = self.delta_pwp0
            elif  int(self.press_period / 5)<= m < int(self.press_period / 4):
                self.Delta_pwp_press_period[m] = 0
            else:
                self.Delta_pwp_press_period[m] = 0#self.delta_pwp0
    ...
    def boundary_violation_press_period(self, A, b, m):
        # 这里的m代表的是应力期的编号
        # 判断边界——顶部边界
        if self.boundary_top[1] == 1:  # 第一类边界
            A[0, 0] = 1.0
            b[0] = self.boundary_top[0] + (self.z0+self.dz/2) * self.rw + self.Delta_pwp_press_period[m]
        elif self.boundary_top[1] == 2:  # 第二类边界
            A[0, 1] = 1.0
            A[0, 0] = -1.0
            b[0] = self.boundary_top[0] * self.dz
        ...
        if self.boundary_bottom[1] == 1:  # 第一类边界
            A[-1, -1] = 1.0
            b[-1] = self.boundary_bottom[0] + (self.z0-self.dz / 2) * self.rw + self.Delta_pwp_press_period[m]
        elif self.boundary_bottom[1] == 2:  # 第二类边界
            A[-1, -2] = -1.0
            A[-1, -1] = 1.0
            b[-1] = self.boundary_bottom[0] * self.dz
